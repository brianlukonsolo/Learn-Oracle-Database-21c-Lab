...
docker compose exec oracle-db bash -lc "sqlplus -s / as sysdba <<< \"SELECT value FROM v\$parameter WHERE name='remote_listener';\""
```
...
- ✋ **Kill the listener** (`docker compose stop listener`) and try `deploy.sh`
  ...
  laid bare.
- 🔁 **Force a re‑register:** `docker compose exec oracle-db bash -lc "sqlplus -s / as sysdba <<< 'ALTER SYSTEM REGISTER;'"`.
